...
export default new class SeaDex extends AbstractSource {
  ...
  async single ({ anilistId, titles, episodeCount }) {
    ...
    const res = await fetch(`${this.url}?page=1&perPage=1&filter=(alID="${anilistId}"%26%26trs.dualAudio?=true)&skipTotal=1&expand=trs`)
